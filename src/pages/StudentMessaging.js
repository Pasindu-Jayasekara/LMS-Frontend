...
import { FaPaperPlane, FaSearch, FaUserTie } from 'react-icons/fa';
...
const StudentMessaging = () => {
    ...
    useEffect(() => {
        const fetchContacts = async () => {
            try {
                const res = await axios.get(`http://localhost:5000/api/messages/student-contacts/${STUDENT_ID}`);
    ...
    useEffect(() => {
        ...
        const fetchConversation = async () => {
            ...
            try {
                const res = await axios.get(`http://localhost:5000/api/messages/conversation/${STUDENT_ID}/${selectedContact.contact_id}`);
                ...
                <div style={styles.messagingContainer}>
                    ...
                    <div style={styles.contactsCol}>
                        <div style={styles.contactsHeader}>
                            <h2 style={styles.contactsTitle}>My Teachers</h2>
                            ...
                            {filteredContacts && filteredContacts.length > 0 ? filteredContacts.map(c => (
                                <div 
                                    ...
                                    style={{
                                        ...
                                        borderLeft: selectedContact?.contact_id === c.contact_id ? '4px solid #2563EB' : '4px solid transparent'
                                    }}
                                    onClick={() => setSelectedContact(c)}
                                >
                                    <div style={styles.avatar}>
                                        <FaUserTie color="#fff" size={18} />
                                    </div>
                                ...
                                <p style={styles.emptyContacts}>No teachers available.</p>
                            )}
                        </div>
                    </div>
                        ...
                        {selectedContact ? (
                            <>
                                ...
                                <div style={styles.messageArea}>
                                    {messages?.map(msg => {
                                        const isMine = msg.sender_id === STUDENT_ID;
                                        return (
                            ...
                            <div style={styles.emptyChat}>
                                ...
                                <p style={styles.emptyChatDesc}>Select a teacher to start messaging.</p>
                            </div>
                        )}
                    </div>
...
const styles = {
    ...
    avatar: {
        width: '40px', height: '40px', borderRadius: '50%', backgroundColor: '#6366F1', display: 'flex', justifyContent: 'center', alignItems: 'center', flexShrink: 0, marginRight: '15px'
    },
    ...
    sendButton: {
        width: '46px', height: '46px', borderRadius: '50%', backgroundColor: '#2563EB', color: '#fff', border: 'none',
        display: 'flex', justifyContent: 'center', alignItems: 'center', cursor: 'pointer', flexShrink: 0, transition: 'background-color 0.2s', ':hover': { backgroundColor: '#1D4ED8' }
    },
